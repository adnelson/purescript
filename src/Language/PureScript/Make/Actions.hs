...
checkForeignDecls
  :: forall m ann
   . (MonadBaseControl IO m, MonadWriter MultipleErrors m, MonadError MultipleErrors m)
  => CF.Module ann -> FilePath -> m ()
checkForeignDecls m path = do
  jsStr <- liftBase $ LB.readFile path
  ...
  errorParsingModule :: Bundle.ErrorMessage -> m a
  ...
  errorInvalidForeignIdentifiers :: [String] -> m a
  errorInvalidForeignIdentifiers =
