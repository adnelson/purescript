...
createTemporaryModule exec st val =
  let
    ...
    effImport     = (P.someModuleNamed effModuleName, P.Implicit, Just (P.ModuleName [P.ProperName "$Effect"]))
    supportImport = (P.someModuleNamed (fst (psciInteractivePrint st)), P.Implicit, Just (P.ModuleName [P.ProperName "$Support"]))
